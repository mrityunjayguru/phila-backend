        ...
        <div class="page-title-heading">
            <div>
                <div class="page-title-head center-elem">
                    ...
                    <span class="d-inline-block">Landing Page</span>
                </div>
            </div>
        </div>
            ...
            <div class="page-title-subheading opacity-10">
                <nav class="" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"> <a>Landing Page</a></li>
                    </ol>
                </nav>
            </div>
        </div>
...
<div class="main-card mb-3 card">
    <div class="card-header-tab card-header">
        <div class="card-header-title font-size-lg text-capitalize font-weight-normal">Landing Page List</div>
        <div class="btn-actions-pane-right text-capitalize">
            ...
            @if(Settings::UserPermission('stop-create'))
                @php $pagelimit = DB::table('landing_page_limit')->first(); @endphp
                @if($pagelimit->limit <= $data)
                    <button class="btn-wide btn-outline-2x mr-md-2 btn btn-outline-focus btn-sm" onclick="alert('Please contact with admin for add new page')">
                        {{trans('common.add_new')}} </button>
                @else
                    <a class="btn-wide btn-outline-2x mr-md-2 btn btn-outline-focus btn-sm" href="{{ route('page-create') }}">
...
function getData(page = 1) {
    ...
    var response = adminAjax('{{route("page-list")}}', data);
        ...
        if (response.data.length > 0) {
            ...
            $.each(response.data, function(index, value) {
                ...
                htmlData += '<tr>' +
                    ...
                    '<td>' + value.title + '</td>' +
                    '<td>' + value.route_length + '</td>' +
                    '<td>' + value.route_time + '</td>' +
                    '<td>' + value.number_of_stops + '</td>' +
                    '<td>' + value.status + '</td>' +
