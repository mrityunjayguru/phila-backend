							...
							<div class="position-relative form-group">
								<div>
									<div class="custom-radio custom-control">
										<input type="radio" id="tour" value="tour" name="typeRadio" class="custom-control-input">
										<label class="custom-control-label" for="tour">Tour</label>
									</div>
									<div class="custom-radio custom-control">
										<input type="radio" id="fairmount_park_loop" value="fairmount_park_loop" name="typeRadio" class="custom-control-input">
										<label class="custom-control-label" for="fairmount_park_loop">Fairmount Park Loop</label>
									</div>
									<div class="custom-radio custom-control">
										<input type="radio" id="mix" value="mix" name="typeRadio" class="custom-control-input">
										<label class="custom-control-label" for="mix">Mix</label>
									</div>
									<div class="custom-radio custom-control">
										<input type="radio" id="all" value="all" name="typeRadio" class="custom-control-input" checked>
...
@section('content')
			...
			<div class="page-title-heading">
				<div>
					<div class="page-title-head center-elem">
						...
						<span class="d-inline-block">Stops</span>
					</div>
				</div>
			</div>
				...
				<div class="page-title-subheading opacity-10">
					<nav class="" aria-label="breadcrumb">
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item"> <a>Stops</a></li>
						</ol>
					</nav>
				</div>
			</div>
	...
	<div class="main-card mb-3 card">
		<div class="card-header-tab card-header">
			<div class="card-header-title font-size-lg text-capitalize font-weight-normal">Stop List</div>
			<div class="btn-actions-pane-right text-capitalize">
				...
				<!--<a class="btn-wide btn-outline-2x mr-md-2 btn btn-outline-focus btn-sm" href="{{ route('stops.create') }}" > {{trans('common.add_new')}} </a>-->
				@endif
			</div>
		</div>
		<div class="card-body">
...
<script>
	...
	function getData(page = 1){
		...
		var response = adminAjax('{{route("ajax.stop.list")}}', data);
	...
	function deleteThis(item_id = ''){
		alert('Please contact developer to perform this action'); return false;
		
		if(confirm("{{trans('common.delete_confirm')}}")){
			var data = new FormData();
			data.append('item_id', item_id);
			var response = adminAjax('{{route("ajax.delete.stop")}}', data);
			if(response.status == '200'){
