			...
			<div class="page-title-heading">
				<div>
					<div class="page-title-head center-elem">
						...
						<span class="d-inline-block">{{ trans(strtolower($role).'.edit') }}</span>
					</div>
				</div>
			</div>
				...
				<div class="page-title-subheading opacity-10">
					<nav class="" aria-label="breadcrumb">
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item"> <a href="{{route('user.management',[$role])}}">{{ trans(strtolower($role).'.plural') }}</a></li>
							<li class="active breadcrumb-item" aria-current="page">{{ trans(strtolower($role).'.edit') }}</li>
						</ol>
					</nav>
				</div>
			</div>
	...
	<div class="main-card mb-3 card">
		<div class="card-body">
			...
			<form class="" action="javascript:void(0);" onsubmit="saveData();">
				...
				<hr>
				@include('backend.user-management.'. $role .'.user-info')
...
<script>
	...
	function getPermissions(){
		var data = new FormData();
		data.append('user_id', '{{$data->id}}');
		var response = adminAjax('{{route("ajax.user.permissions.list")}}', data);
		if(response.status == '200'){
			var htmlData = '';
