					...
					<div class="row">
						<div class="col-md-6">
							<div class="input-group">
								<input id="ticket_number" placeholder="Code number" step="1" type="text" class="form-control">
							</div>
						</div>
						<!--<div class="col-md-6">
							<div class="input-group">
								<input id="customer_name" placeholder="Customer Name" step="1" type="text" class="form-control">
							</div>
						</div>-->
					</div>
					<br>
...
@section('content')
			...
			<div class="page-title-heading">
				<div>
					<div class="page-title-head center-elem">
						...
						<span class="d-inline-block">Code</span>
					</div>
				</div>
			</div>
	...
	<div class="main-card mb-3 card">
		<div class="card-header-tab card-header">
			<div class="card-header-title font-size-lg text-capitalize font-weight-normal">Code List</div>
			<div class="btn-actions-pane-right text-capitalize">
				...
				@if(Settings::UserPermission('ticket-create'))
				<a class="btn-wide btn-outline-2x mr-md-2 btn btn-outline-focus btn-sm" href="{{ route('codes.create',$page_id) }}" > {{trans('common.add_new')}} </a>
				@endif
			</div>
		</div>
		<div class="card-body">
...
<script>
	...
	function getData(page = 1){
		...
		data.append('ticket_number', $('#filterBox #ticket_number').val());
		data.append('customer_name', $('#filterBox #customer_name').val());
		data.append('status', $('input[name="statusRadio"]:checked').val());
		var response = adminAjax('{{route("ajax.codes.list")}}', data); 
		if(response.status == '200'){
			...
			if(response.data.length > 0){
				...
				$.each(response.data, function( index, value ) {
					...
					htmlData+= '<tr style="'+ value.is_expired +'">'+
	...
	function deleteThis(item_id = ''){
		if(confirm("{{trans('common.delete_confirm')}}")){
			...
			var response = adminAjax('{{route("ajax.delete.code")}}', data);
			if(response.status == '200'){
				window.location.href = "/backend/codes/" + page_id;
