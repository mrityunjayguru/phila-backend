...
@section('css')
	<style>#mymap{width: 100%; height: 720px;}</style>
@endsection
...
@section('js')
	...
	<script>
		...
		var myOptions = {
			zoom: 14,
			center: new google.maps.LatLng(39.958053, -75.174008),
		...
		function InitializeMap() {
			...
			// Create markers.
			/*var data = new FormData();
			var response = runAjax(SITE_URL +'/plcs', data);
			if(response.status == '200'){
				if(response.data.length > 0){
					$.each(response.data, function( index, value ) {
						const marker = new google.maps.Marker({
						  position: new google.maps.LatLng(value.latitude,value.longitude),
						  icon: 'http://citysightseeingphila.com/citysight/default/places/'+ value.type +'.svg',
						  map: map,
						  title: 'Hello world!!',
						});
					});
		...
		function updateMarker() {
			...
			data.append('device_type', 'Bus,Trolley-Blue-Route');
