    ...
    public function index(Request $request){
		...
		try{
			...
			if($request->latitude && $request->longitude){
				...
				->where("buses.status", "active")->where("buses.device_type", "Bus")->orderBy("distance", "ASC");
			}
			// For All Live Buses
			else{
				$query = Bus::where(['status'=>'active', 'device_type'=>'Bus']);
			}
			...
			if($request->is_multiple == 1){
				...
				$query = $query->where("buses.deleted_at", null)->first();
				return $this->sendResponse(new TrackBusList($query),trans('customer_api.data_found_success'));
			}
